<template>
  <div class="mb3 mb4-l ba b--black-10 drop-shadow">
    <div class="pa3 bg-white">
      <h1 class="ma0 dark-gray f4">{{buildpack.name}}</h1>
      <hr/>
      <div class="mt3 mb1">
        <h4 class="fw5 tracked ttu f7 ma0 black-50">Latest</h4>
        <h2 class="fw5 f4 mb0 mt2">
          <router-link class="blue link no-underline" :to="detailUrl">
            {{ latestVersion }}
          </router-link>
        </h2>
      </div>
    ...
    <div class="ph3 pv2 bt b--black-10">
      <a :href="githubUrl" target="_blank" class="link f6 mid-gray" :disabled="!latestVersion">
...
export default {
  ...
  computed: {
    detailUrl () {
      return `buildpacks/${this.buildpack.id}/${this.latestVersion}`
    },
    githubUrl () {
